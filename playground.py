...
def input_fn_predict(value):
    dataset = Dataset.from_tensor_slices({'personen': tf.constant([value])})
    dataset = dataset.batch(1).repeat(1)
    return dataset.make_one_shot_iterator().get_next()


def predict(value):
    predict_result = linear_regressor.predict(input_fn=lambda: input_fn_predict(value))

    predict_result = np.array([item['predictions'][0] for item in predict_result])
    for i in predict_result:
        print("Predicted value for input %0.2f: %0.8f" % (value, i))
